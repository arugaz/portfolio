use crate::component::animate::{DivContent, HomeCard, SectionContent, SpanFade};
...
pub fn home_page() -> Html {
  html! {
    <main class="relative flex w-full flex-col items-center justify-evenly py-40 md:h-screen md:flex-row md:overflow-hidden">
      ...
      <DivContent class="relative order-2 flex w-1/2 justify-end md:order-1 md:w-1/3">
          <HomeCard class="absolute mt-20 h-64 w-full rounded-xl bg-[#464646] md:h-72 md:w-64 lg:h-80" transform="rotate(-12deg)" />
          <HomeCard class="absolute mt-20 h-64 w-full rounded-xl bg-secondary-light md:h-72 md:w-64 lg:h-80" transform="rotate(-3deg)" />
          <HomeCard class="relative mt-20 h-64 w-full rounded-xl bg-primary-light md:h-72 md:w-64 lg:h-80" transform="rotate(2deg)">
      ...
      <SectionContent class="relative order-1 w-1/3 md:order-2">
