use crate::{
  component::animate::{HeaderFade, Line},
  data::page::PAGES,
};
...
pub fn header(props: &Props) -> Html {
  ...
  html! {
    <HeaderFade class="fixed left-0 -top-12 z-40 flex w-full justify-between pt-24 md:top-0">
      <div class="flex w-1/2 items-baseline">
        <div class="w-3/5 md:w-2/5">
          <Line width="25%" class="mb-1 h-1 rounded-tr-md rounded-br-md bg-primary-light" />
          <Line width="100%" class="mb-1 h-1 rounded-tr-md rounded-br-md bg-primary-light" />
          <Line width="67%" class="h-1 rounded-tr-md rounded-br-md bg-primary-light" />
        </div>
      </div>
